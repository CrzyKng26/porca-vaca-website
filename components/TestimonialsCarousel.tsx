...
export default function TestimonialsCarousel({ initialTestimonials }: { initialTestimonials: Testimonial[] }) {
          ...
          <AnimatePresence mode="wait" custom={direction}>
            ...
            >
              ...
              <div className="mt-8 flex flex-col items-center gap-1 pointer-events-none">
                ...
                <span className="text-body-sm text-bone font-medium">{testimonials[current]?.customer_name}</span>
                <span className="text-label tracking-widest uppercase text-bone/40">{testimonials[current]?.role}{testimonials[current]?.location ? `, ${testimonials[current].location}` : ''}</span>
              </div>
            </motion.div>
          </AnimatePresence>
        </div>
